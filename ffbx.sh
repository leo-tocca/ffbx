...
# Copyright (C) 2014-2025 Thomas Szteliga <ts@websafe.pl>, <https://websafe.pl/>§
# Copyright (C) 2025- Leonardo Toccafondi <leotocca@protonmail.com>§
#
...
# Check for sqlite3
if ! command -v ${CMD_SQLITE3} &> /dev/null; then
    echo "Error: sqlite3 is not installed. Please install sqlite3 to use this script."
    exit 1
fi
...
if [ -z "${1}" ]; then
    ...
    if [ -z "${found_db_places_paths}" ]; then
...
debug "db_places_paths_were_autodiscovered" "${db_places_paths_were_autodiscovered}"

# If an output file is specified, redirect output to that file
if [ -n "${output_file}" ]; then
    exec > "${output_file}" 2>&1
fi

for db_places_path in ${found_db_places_paths}; do
    ...
    bookmarks_data=$(
        ...
        "SELECT lastModified, fk FROM moz_bookmarks WHERE type=1 ORDER BY lastModified"
    )

    bookmarks_places_ids=$(
        echo "${bookmarks_data}" | ${CMD_CUT} -d'|' -f2 | ${CMD_UNIQ}
    )

    debug "bookmarks_places_ids ${bookmarks_places_ids}"

    for bookmark_places_id in ${bookmarks_places_ids}; do
        ...
        bookmark_url=$(
            ${CMD_SQLITE3} "${db_places_path}" \
                "SELECT url FROM moz_places WHERE id=${bookmark_places_id}" \
                | ${CMD_TR} -d "\n" | ${CMD_TR} -d "\r"
        )
        ...
        bookmark_tags_ids=$(
            ${CMD_SQLITE3} "${db_places_path}" \
                "SELECT parent FROM moz_bookmarks WHERE fk=${bookmark_places_id} AND title IS NULL"
        )

        debug "bookmark_tags_ids ${bookmark_tags_ids}"

        # Retrieve comma-separated list of tags assigned to the current bookmark:
        bookmark_tags=$(
            for bookmark_tag_id in ${bookmark_tags_ids}; do
                ${CMD_SQLITE3} "${db_places_path}" \
                    "SELECT title FROM moz_bookmarks WHERE id=${bookmark_tag_id}"
            done | ${CMD_TR} "\n" "${FFBX_ITEM_SEPARATOR}"
        )
        ...
        bookmark_title=$(
            ${CMD_SQLITE3} "${db_places_path}" \
                "SELECT title FROM moz_bookmarks WHERE fk=${bookmark_places_id} AND title!='' LIMIT 1"
        )
        ...
        bookmark_last_modification=$(
            ${CMD_SQLITE3} "${db_places_path}" \
                "SELECT lastModified FROM moz_bookmarks WHERE fk=${bookmark_places_id} ORDER BY lastModified DESC LIMIT 1"
        )
        ...
        bookmark_folder_id=$(
            ${CMD_SQLITE3} "${db_places_path}" \
                "SELECT parent FROM moz_bookmarks WHERE type=1 AND fk=${bookmark_places_id} ORDER BY id ASC LIMIT 1"
        )
        ...
        bookmark_folder_name=$(
            ${CMD_SQLITE3} "${db_places_path}" \
                "SELECT title FROM moz_bookmarks WHERE id=${bookmark_folder_id}"
        )
        ...
        if [ "${db_places_paths_were_autodiscovered}" = "yes" ]; then
